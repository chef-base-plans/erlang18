title 'Tests to confirm erlang18 works as expected'

plan_origin = ENV['HAB_ORIGIN']
plan_name = input('plan_name', value: 'erlang18')

control 'core-plans-erlang18-works' do
  impact 1.0
  title 'Ensure erlang18 works as expected'
  desc '
  ...
  plan_pkg_version = plan_installation_directory.stdout.split("/")[5]
  {
    ...
    if(script)
      Tempfile.open('foo') do |f|
        f << script
        sleep(1) # ensure file is present
        command_under_test = command("#{command_full_path} #{f.path}")
      end
    else
